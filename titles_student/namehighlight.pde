class NameHighlight {
 ...
 PVector destination;
  ...
  boolean enterBoxes(float clrLimit) {
    ...
    for (HlGridBox box : gridBoxes) {
      ...
      if (stateComplete) stateComplete = box.clrStateComplete(box.boxClr, box.destClr, clrLimit);
    }
  ...
  boolean exitBoxes(float clrLimit) {
    boolean stateComplete = true;
   ...
   for (HlGridBox box : gridBoxes) {
    ...
    if (stateComplete) stateComplete = box.clrStateComplete(box.boxClr, box.initClr, clrLimit);
   } 
